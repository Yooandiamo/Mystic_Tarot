...
const App: React.FC = () => {
  ...
  const handleSpreadSelect = (spread: Spread) => {
    ...
    // If auto, we can technically skip, but let's show the fan anyway for manual feel
    // unless strictly auto is requested.
  ...
  const drawCardAtIndex = (deckIndex: number) => {
    ...
    // Remove from deck visually (optional, but good for UX)
    const newDeck = [...deck];
  ...
  const handleAutoDraw = () => {
     ...
     for(let i=0; i<selectedSpread.cardCount; i++) {
        ...
        currentDeck.splice(randomIndex, 1); // remove to avoid dupe
        selected.push({
           ...card,
           isReversed: Math.random() > 0.8,
           positionIndex: i,
           positionName: selectedSpread.positions[i] || `位置 ${i+1}`
        });
     }
  ...
  // Re-added Reveal All Functionality
  const handleRevealAll = () => {
    ...
    const revealNext = () => {
      if (current < total) {
        setRevealedCount(prev => prev + 1);
        current++;
        setTimeout(revealNext, 300); // 300ms delay between flips
      }
    };
    revealNext();
  };
    ...
    <div className="w-full max-w-lg mx-auto space-y-6 animate-fade-in pt-10 px-6">
      ...
      <div className="flex flex-wrap gap-2 justify-center">
        {QUESTION_TEMPLATES.slice(0, 3).map((q, i) => (
          <button
            ...
            className="px-3 py-1.5 bg-slate-800/50 border border-slate-700 rounded-full text-xs text-slate-400 hover:text-amber-300 hover:border-amber-500/50 transition-all"
          >
            {q}
          </button>
        ))}
      ...
      <div className="pt-8 flex justify-center">
        <button
          ...
          className="w-full py-4 bg-gradient-to-r from-indigo-600 to-purple-600 disabled:opacity-50 disabled:cursor-not-allowed rounded-xl text-white font-semibold transition-all hover:shadow-[0_0_20px_rgba(99,102,241,0.4)]"
        >
          下一步
        </button>
      </div>
  ...
  const renderSpreadSelect = () => (
    <div className="w-full max-w-4xl mx-auto px-4 py-8 animate-fade-in">
       {/* 1. New Prominent Draw Mode Selection */}
       <div className="mb-12">
         ...
         <div className="flex flex-col sm:flex-row justify-center gap-6 max-w-2xl mx-auto">
            <button
              ...
              className={`flex-1 p-6 rounded-2xl border transition-all relative overflow-hidden group
                ${drawMode === DrawMode.MANUAL 
                  ? 'bg-[#251f47] border-amber-500 shadow-[0_0_20px_rgba(245,158,11,0.25)]' 
                  : 'bg-[#1a1638] border-slate-800 hover:border-slate-600 hover:bg-[#1f1a42]'}`}
            >
              <div className="flex items-center gap-4 relative z-10">
                <div className={`w-12 h-12 rounded-full flex items-center justify-center text-xl transition-colors
                  ${drawMode === DrawMode.MANUAL ? 'bg-amber-500/20 text-amber-400' : 'bg-slate-800 text-slate-500'}`}>
                   <HandIcon />
                </div>
                <div className="text-left">
                   <span className={`block font-bold text-lg mb-1 ${drawMode === DrawMode.MANUAL ? 'text-amber-100' : 'text-slate-300'}`}>沉浸仪式 (手动)</span>
                   <span className="text-xs text-slate-400">亲自洗牌与抽取，建立更深连接</span>
                </div>
              </div>
              {/* Highlight effect */}
              {drawMode === DrawMode.MANUAL && <div className="absolute top-0 right-0 w-20 h-20 bg-amber-500/10 rounded-bl-full blur-xl"></div>}
            </button>

            <button
              onClick={() => setDrawMode(DrawMode.AUTO)}
              className={`flex-1 p-6 rounded-2xl border transition-all relative overflow-hidden group
            ...
            >
              <div className="flex items-center gap-4 relative z-10">
                <div className={`w-12 h-12 rounded-full flex items-center justify-center text-xl transition-colors
                  ${drawMode === DrawMode.AUTO ? 'bg-amber-500/20 text-amber-400' : 'bg-slate-800 text-slate-500'}`}>
                   <MagicIcon />
                </div>
                <div className="text-left">
                   <span className={`block font-bold text-lg mb-1 ${drawMode === DrawMode.AUTO ? 'text-amber-100' : 'text-slate-300'}`}>快速指引 (自动)</span>
                   <span className="text-xs text-slate-400">交由宇宙指引，一键完成抽取</span>
                </div>
              </div>
               {/* Highlight effect */}
               {drawMode === DrawMode.AUTO && <div className="absolute top-0 right-0 w-20 h-20 bg-amber-500/10 rounded-bl-full blur-xl"></div>}
            </button>
         </div>
       </div>
       ...
       <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          {SPREADS.map(spread => (
            <button
              ...
              className={`relative group p-6 rounded-xl border transition-all text-left flex flex-col h-full
                ${selectedSpread.id === spread.id 
                  ? 'bg-[#251f47] border-amber-500 shadow-[0_0_15px_rgba(245,158,11,0.2)]' 
                  : 'bg-[#1a1638] border-slate-800 hover:border-slate-600'}`}
            >
               <div className="mb-4 w-10 h-10 rounded-lg bg-slate-800 flex items-center justify-center text-amber-500 font-serif font-bold text-lg border border-slate-700">
                 {spread.cardCount}
               </div>
               ...
               <div className="mt-auto pt-4 border-t border-white/5 flex items-center text-xs text-slate-500">
        ...
        {drawMode === DrawMode.AUTO && (
           <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 z-30">
              <button 
                 ...
                 className="w-32 h-32 rounded-full bg-amber-500/10 border border-amber-500/50 backdrop-blur-sm flex flex-col items-center justify-center gap-2 hover:bg-amber-500/20 transition-all animate-pulse hover:animate-none cursor-pointer"
              >
                 <MagicIcon />
                 <span className="text-amber-200 text-sm font-bold">洗牌 & 抽取</span>
              </button>
           </div>
        )}
  ...
  const renderReveal = () => (
    <div className="w-full h-full flex flex-col items-center justify-center animate-fade-in pt-10 pb-20 px-4">
      ...
      {/* Re-added Reveal All Button */}
      {revealedCount < drawnCards.length && (
         <div className="flex justify-center mb-8">
            <button 
              onClick={handleRevealAll}
              className="flex items-center gap-2 px-5 py-2 rounded-full bg-slate-800 border border-slate-700 text-amber-500 hover:bg-slate-700 hover:border-amber-500/50 transition-all text-sm font-bold uppercase tracking-wide"
            >
               <EyeIcon />
               <span>一键翻牌</span>
            </button>
        ...
        {drawnCards.map((card, index) => (
          <div key={card.id} className="flex flex-col items-center gap-4">
            <div className={`transition-all duration-700 ${index <= revealedCount ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'}`}>
               <span className="text-xs text-amber-500 uppercase tracking-widest font-bold mb-2 block text-center">
                 {card.positionName.split('/')[0]}
               </span>
            </div>
            ...
            <Card 
              ...
              size="md"
              className={`transition-all duration-500 ${index === revealedCount ? 'ring-4 ring-amber-500/30 scale-110 z-10' : 'scale-100 opacity-90'}`}
            />
          </div>
        ))}
      </div>
      ...
      {revealedCount === drawnCards.length && (
        <div className="fixed bottom-10 left-0 right-0 flex justify-center z-50 animate-bounce-in">
          <button
            ...
            disabled={loadingAI}
            className="group relative px-12 py-4 bg-[#0f0c29] border border-amber-500/50 rounded-full overflow-hidden shadow-[0_0_30px_rgba(245,158,11,0.3)]"
          >
      ...
      <div className="w-full max-w-2xl mx-auto pb-20 animate-fade-in relative">
           ...
           <div className="flex justify-center gap-4 overflow-x-auto pb-2 scrollbar-hide">
              {reading.cards.map((card, idx) => (
                 <div key={idx} className="flex flex-col items-center min-w-[60px]">
                    ...
                    <span className="text-[10px] text-slate-400 text-center leading-tight w-full truncate px-1">
                       {card.positionName.split('/')[0]}
                    </span>
                 </div>
              ))}
        ...
        <div className="p-6 space-y-6">
           ...
           <div className="space-y-2">
              ...
              <div className="bg-[#1e1b4b]/60 border border-indigo-500/30 rounded-xl p-5 text-indigo-100 leading-relaxed shadow-lg backdrop-blur-sm">
                 {summary}
              </div>
           </div>
           ...
           <div className="space-y-4">
              ...
              {reading.cards.map((card, idx) => {
                 // DIRECT INDEX MATCHING - No fuzzy search needed because the service ensures alignment
                 ...
                 return (
                    <div key={idx} className="bg-slate-900/40 rounded-xl border border-slate-800 overflow-hidden">
                       <div className="bg-slate-800/50 px-4 py-3 flex items-center justify-between border-b border-slate-800">
                          ...
                          <span className={`text-[10px] px-2 py-0.5 rounded border ${card.isReversed ? 'border-red-900/50 text-red-400 bg-red-900/10' : 'border-green-900/50 text-green-400 bg-green-900/10'}`}>
                             {card.isReversed ? '逆位 (Reversed)' : '正位 (Upright)'}
                          </span>
                       </div>
                       <div className="p-4">
                          <p className="text-slate-300 text-sm leading-relaxed">
                             {analysis ? analysis.meaning : "暂无详细解析。"}
                          </p>
                       </div>
                    </div>
                 );
           ...
           <button onClick={resetApp} className="w-full py-3 rounded-full border border-slate-700 text-slate-400 hover:text-white hover:border-slate-500 transition-all text-sm uppercase tracking-widest">
              开始新的占卜
           </button>
        </div>
      </div>
